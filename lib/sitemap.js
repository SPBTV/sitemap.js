...
function SitemapItem(conf) {
  ...
  this.video = conf['video'] ? new VideoItem(conf['video']) : null;
}

function VideoItem(conf) {
    this.thumbnail_loc = conf['thumbnail_loc'];
    this.title = conf['title'];
    this.description = conf['description'];
    this.content_loc = conf['content_loc'];
}

VideoItem.prototype.toXML = function () {
    return this.toString();
}

VideoItem.prototype.toString = function () {
    // result xml
    var xml = '<video:video> {thumbnail_loc} {title} {description} {content_loc} </video:video>'
    // xml property
    , props = ['thumbnail_loc', 'title', 'description', 'content_loc']
...
SitemapItem.prototype.toString = function () {
      // result xml
  var xml = '<url> {loc} {lastmod} {changefreq} {priority} {video} </url>'
      // xml property
    , props = ['loc', 'lastmod', 'changefreq', 'priority', 'video']
      // property array size (for loop)
    , ps = props.length
      // current property name (for loop)
    , p
  ...
  while ( ps-- ) {
    ...
    if (this[p]) {
      buf = '<'+p+'>'+this[p]+'</'+p+'>';
      if (this[p].toXML !== undefined) {
          buf = this[p].toXML();
      }
      xml = xml.replace('{'+p+'}', buf);
    } else {
      xml = xml.replace('{'+p+'}', '');
    }
  }
